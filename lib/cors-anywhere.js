...
function getHandler(options, proxy) {
  ...
  return function(req, res) {
    // CUIT lookup endpoint: fetch and return https://www.cuitonline.com/detalle/CUIT/
    // Matches requests like: /cuit/20345678901
    var cuitMatch = req.url && req.url.match(/^\/cuit\/([^\/\?]+)(?:[\/\?]|$)/i);
    if (req.method === 'OPTIONS' && cuitMatch) {
      res.writeHead(200, {'Access-Control-Allow-Origin': '*'});
      res.end();
      return;
    }

    if (req.method === 'GET' && cuitMatch) {
      var cuit = cuitMatch[1];
      var debugUrl = 'https://www.cuitonline.com/detalle/' + encodeURIComponent(cuit) + '/';
      var client = debugUrl.indexOf('https://') === 0 ? require('https') : require('http');
      client.get(debugUrl, function(debugRes) {
        var statusCode = debugRes.statusCode || 200;
        var contentType = debugRes.headers['content-type'] || '';
        var body = '';
        debugRes.setEncoding('utf8');
        debugRes.on('data', function(chunk) { body += chunk; });
        debugRes.on('end', function() {
